...
void *DetourHooking::Align(const void *const addr,
                           const std::size_t alignment) {
  return (void *)(((std::size_t)addr) & ~(alignment - 1));
}

std::size_t DetourHooking::PointerDistance(const void *const a,
                                           const void *const b) {
  return std::abs(reinterpret_cast<const char *>(b) -
                  reinterpret_cast<const char *>(a));
}

void DetourHooking::Protect(const void *const addr, const std::size_t length,
                            const int prot) {
...
void DetourHooking::WriteRelJmp(void *const location,
  ...
  void *const jmpTarget = reinterpret_cast<void *const>(
      reinterpret_cast<const char *const>(target) -
      (reinterpret_cast<char *const>(location) +
       relJmpLength)); // Jumps always start at the rip, which has already
                       // increased
  std::memcpy(jmpInstruction + 1, &jmpTarget, sizeof(int32_t));
  std::memcpy(location, jmpInstruction, relJmpLength);
}

void DetourHooking::WriteAbsJmp(void *const location,
                                const void *const target) {
